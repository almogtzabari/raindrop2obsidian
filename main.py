...
parser = argparse.ArgumentParser(description="Raindrop.io highlights to Obsidian.md")

parser.add_argument(
    "-c", "--config-filename", type=str, default="config.yaml", dest="config_filename", help="Path to config file."
)
...
def sync_raindrop(raindrop, md_filename: str, raindrop_template: list[str], highlight_template: list[str]) -> None:
    ...
    if not os.path.isfile(md_filename):
        ...
        with open(md_filename, "r", encoding="utf-8") as f:
            lines = f.readlines()
            try:
                last_update_line_num = list(filter(lambda item: item[1].startswith("last_update"), enumerate(lines)))[
                    0
                ][0]
                last_update_line = lines[last_update_line_num]
                note_last_update = last_update_line.split(": ")[-1].strip()
                lines[last_update_line_num] = lines[last_update_line_num].replace(
                    note_last_update, raindrop.last_update
                )
            except:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor:
            for collection in collections:
                for raindrop in collection.fetch_all_raindrops():
                    logger.info(
                        f"Syncing {fix_problematic_strings(collection.title)}: {fix_problematic_strings(raindrop.title)}"
                    )
                    collection_dir = os.path.join(
                        os.path.realpath(config["target_dir"]), find_valid_filename(collection.title)
                    )
                    os.makedirs(collection_dir, exist_ok=True)
                    md_filename = (
                        os.path.join(collection_dir, f"{raindrop.id} - " + find_valid_filename(raindrop.title)) + ".md"
                    )
                    executor.submit(
                        sync_raindrop,
                        raindrop=raindrop,
